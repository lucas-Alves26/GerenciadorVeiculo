...
SELECT M.MAN_INT_ID,M.MAN_DOU_VALOR,M.MAN_TIME_HORA_INI,MAN_TIME_HORA_FIN,V.VEI_STR_PLACA,V.VEI_STR_MODELO,V.VEI_DOUBLE_KM, P.EMP_INT_ID, P.EMP_STR_NOME FROM TBL_MANUTENCAO AS M
INNER JOIN TBL_VEICULO AS V ON M.VEI_INT_ID =  V.VEI_INT_ID
INNER JOIN TBL_EMPRESA AS P ON P.EMP_INT_ID = V.EMP_INT_ID 
INNER JOIN TBL_SERVICO AS S PN S.SERV_INT_ID = 

WHERE M.MAN_INT_ID = 1


--UPDATE TBL_MANUTENCAO SET MAN_TIME_HORA_FIN = '', MAN_DOU_VALOR = '' WHERE MAN_INT_ID = 

--UPDATE TBL_VEICULO SET VEI_STR_SITUACAO = 'Disponivel' WHERE VEI_INT_ID = 

SELECT  DISTINCT M.MAN_INT_ID,V.VEI_STR_PLACA ,V.VEI_STR_MARCA,V.VEI_STR_MODELO,V.VEI_STR_COMBUSTIVEL, CONVERT(VARCHAR(10),M.MAN_DATE_DATA,103) AS Data , V.VEI_DOUBLE_KM FROM TBL_MANUTENCAO AS M
...
WHERE  M.MAN_DOU_VALOR IS NULL
 ...
 INSERT INTO TBL_SERVICO(MAN_INT_ID,SERV_STR_SERVICO,SERV_STR_TIPO_SERV,SERV_STR_OBS)  VALUES(3,'Preventiva','Motor','')

INSERT INTO TBL_MANUTENCAO(VEI_INT_ID,MAN_STR_SERVICO,MAN_STR_TIPO_SERV,MAN_DATE_DATA,MAN_STR_OBS)
VALUES(@idVei,@servico,@tipoServ,@date)
